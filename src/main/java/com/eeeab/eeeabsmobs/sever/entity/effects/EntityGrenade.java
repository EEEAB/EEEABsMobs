...
public class EntityGrenade extends EntityMagicEffects {
    ...
    public void tick() {
        ...
        if (hitresult.getType() != HitResult.Type.MISS) {
            if (!this.level.isClientSide) {
                ...
                this.level.explode(this, DamageSource.explosion(this.caster), null, this.position().x, this.position().y, this.position().z, RADIUS, false, Explosion.BlockInteraction.NONE);
    ...
    protected boolean canHitEntity(Entity entity) {
        if (!this.isAlive() && !this.isPickable()) {
            return false;
        } else {
