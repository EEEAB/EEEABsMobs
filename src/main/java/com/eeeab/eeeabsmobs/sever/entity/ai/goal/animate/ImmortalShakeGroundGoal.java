...
public class ImmortalShakeGroundGoal extends AnimationAI<EntityImmortal> {
    ...
    public void tick() {
        ...
        if (animation == entity.smashGround1Animation) {
            ...
            if (tick == 15) entity.playSound(SoundInit.IMMORTAL_ATTACK.get(), 1.6F, entity.getVoicePitch() + 0.05F);
                ...
                for (LivingEntity entityHit : ShockWaveUtils.doRingShockWave(entity, entity.getPosOffset(false, 1.5F, 0F, -1F), 3.6D, 0F, false, 12)) {
                    boolean hit = entity.immortalHurtTarget(entityHit, false, false, false, false, 1.2F, 0.9F);
            ...
            if (tick == 12) entity.playSound(SoundInit.IMMORTAL_ATTACK.get(), 1.7F, entity.getVoicePitch() + 0.1F);
                ...
                for (LivingEntity entityHit : ShockWaveUtils.doRingShockWave(entity, entity.getPosOffset(false, 1.5F, 0F, -1F), 4D, -0.01F, true, 35)) {
                    entity.immortalHurtTarget(entityHit, false, false, false, false, 1.1F, 1.1F);
                    entity.disableShield(entityHit, 50);
                ...
                ShockWaveUtils.doAdvShockWave(entity, i, 3F, 2, 1.5, 2, false, false, hit -> {
                    if (hit instanceof LivingEntity entityHit) {
                        entity.immortalHurtTarget(entityHit, false, false, false, false, 1.0F, 0.6F);
            ...
            if (tick > 10 && tick < 20) {
                ...
                ShockWaveUtils.doAdvShockWave(entity, i, 3F, 0.7, 1.5, 2, false, false, hit -> {
                    if (hit instanceof LivingEntity entityHit) {
                        entity.immortalHurtTarget(entityHit, false, false, false, false, 1.0F, 0.75F);
            ...
            if (tick == 15) entity.playSound(SoundInit.IMMORTAL_ATTACK.get(), 1.7F, entity.getVoicePitch() - 0.05F);
                ...
                for (LivingEntity entityHit : entity.getNearByLivingEntities(attackDistance)) {
                    ...
                    if (entityHitDistance <= attackDistance && ((entityRelativeAngle >= -20 && entityRelativeAngle <= 90) || (entityRelativeAngle >= 360 - 20 || entityRelativeAngle <= -360 + 90))) {
                        entity.immortalHurtTarget(entityHit, false, entityHit.hasEffect(EffectInit.ERODE_EFFECT.get()), true, false, 0.8F, 1.0F);
            ...
            if (tick == 42) entity.playSound(SoundInit.IMMORTAL_ATTACK.get(), 1.7F, entity.getVoicePitch() + 0.1F);
                ...
                for (LivingEntity entityHit : ShockWaveUtils.doRingShockWave(entity, entity.getPosOffset(false, 4.5F, 0F, -1F), 2.46D, 0F, false, 10)) {
                    ...
                    boolean hit = entity.immortalHurtTarget(entityHit, false, false, false, false, 1.2F, 1.0F);
                    entity.disableShield(entityHit, 50);
    ...
    private void smashGround(Vec3 pos) {
        for (LivingEntity entityHit : ShockWaveUtils.doRingShockWave(entity, pos, 2.8D, 0F, false, 5)) {
            entity.immortalHurtTarget(entityHit, true, false, true, false, 1.1F, 1.2F);
