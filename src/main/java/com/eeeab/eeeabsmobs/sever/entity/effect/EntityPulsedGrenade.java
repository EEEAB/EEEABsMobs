...
public class EntityPulsedGrenade extends EntityMagicEffects {
    private static final float GRAVITY = 0.03F;
    private float radius = 2.5F;

    public EntityPulsedGrenade(EntityType<? extends EntityMagicEffects> entityType, Level level) {
        super(entityType, level);
    }

    public EntityPulsedGrenade(Level level, LivingEntity caster) {
    ...
    public void tick() {
        ...
        if (hitresult.getType() != HitResult.Type.MISS) {
            ...
            float damage = this.caster instanceof Player ? this.getPlayerDamage() : this.getDamage();
            EntityExplode.explode(this.level(), this.position(), this.damageSources().explosion(this, this.caster), this.caster, this.radius, damage);
