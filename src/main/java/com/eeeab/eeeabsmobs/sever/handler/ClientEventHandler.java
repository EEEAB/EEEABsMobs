...
import com.eeeab.eeeabsmobs.client.gui.CombatPromptHandler;
...
public class ClientEventHandler {
    ...
    public void onRenderGuiOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (event.getOverlay().overlay() == VanillaGuiOverlay.PLAYER_LIST.type().overlay()) {
            CombatPromptHandler.renderOverlay(event.getGuiGraphics(), event.getPartialTick());
        }
    }
    ...
    public void onRenderTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            CombatPromptHandler.clientTick();
        }
    }
