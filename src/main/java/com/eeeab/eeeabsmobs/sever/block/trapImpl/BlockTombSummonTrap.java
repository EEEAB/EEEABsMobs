    ...
    protected void active(Level level, BlockPos pos, BlockState state, Entity entity) {
        if (!state.getValue(OPEN) && checkStepOnEntity(entity)) {
            ...
            if (level instanceof ServerLevel server && golem != null) {
                golem.setInitSpawn();
                golem.finalizeSpawn(server, level.getCurrentDifficultyAt(pos),
            ...
            level.playSound(null, pos, SoundInit.IMMORTAL_SHAMAN_PREPARE_SPELL_CASTING.get(), SoundSource.BLOCKS, 1F, 1F);
