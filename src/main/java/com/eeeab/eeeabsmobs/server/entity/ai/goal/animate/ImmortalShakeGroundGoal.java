package com.eeeab.eeeabsmobs.server.entity.ai.goal.animate;
...
public class ImmortalShakeGroundGoal extends AnimationAI<EntityImmortalBoss> {
    private static final int SHAKE_GROUND_RANGE = 20;

    public ImmortalShakeGroundGoal(EntityImmortalBoss entity) {
        super(entity);
    }
    ...
    public void tick() {
        ...
        if (animation == EntityImmortalBoss.SMASH_GROUND_ANIMATION1) {
            ...
            if (tick > 10 && tick < 20) {
                ...
                pursuit(target, ModMathUtils.getTickFactor(tick - 10, 5, true), 3F, 6F, 2F);
            ...
            if (tick > 10 && tick < 30) {
                pursuit(target, ModMathUtils.getTickFactor(tick - 10, 20, true), 5F, 10F, 3F);
            } else if (tick >= 40 && tick <= 45) {
                entity.setYRot(entity.yRotO);
                pursuit(target, ModMathUtils.getTickFactor(tick - 40, 5, true), 3.725F, 7.5F, 7F);
            }
    ...
    private void pursuit(LivingEntity target, float tickFactor, float distance, float maxDistance, float canMoveDistance) {
        ...
        if (target != null) {
            ...
            distanceFactor = ModMathUtils.getTickFactor(Math.min(distanceTo, distance), maxDistance, false);
        }
