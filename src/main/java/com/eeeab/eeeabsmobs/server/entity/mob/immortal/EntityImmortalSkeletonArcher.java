package com.eeeab.eeeabsmobs.server.entity.mob.immortal;

import com.eeeab.eeeabsmobs.server.entity.util.ModEntityUtils;
import net.minecraft.world.entity.EntityType;
import net.minecraft.world.entity.LivingEntity;
import net.minecraft.world.entity.monster.RangedAttackMob;
import net.minecraft.world.level.Level;

public class EntityImmortalSkeletonArcher extends EntityAbsImmortalSkeleton implements RangedAttackMob {
    public EntityImmortalSkeletonArcher(EntityType<? extends EntityAbsImmortal> type, Level level) {
        super(type, level);
    }

    @Override
    protected void registerCustomGoals() {
        this.addRangeAI(this);
        super.registerCustomGoals();
    }

    @Override
    public void tick() {
        super.tick();
        if (this.getTarget() != null && !isNoAi() && this.isActive() && this.isAlive()) {
            LivingEntity target = getTarget();
            if (attackTick == 0 && getSensing().hasLineOfSight(target)) {
                attacking = true;
            }
            if (attacking && getSensing().hasLineOfSight(target) && canAttack(target)) {
                if (targetDistance < 1.5 || targetDistance < 2.5 && ModEntityUtils.checkTargetComingCloser(this, target)) {
                    this.stopUsingItem();
                    this.playAnimation(MELEE_ANIMATION1);
                    attackTick = 20;
                    attacking = false;
                }
            }
        } else {
            attacking = false;
        }
    }

    @Override
    public void performRangedAttack(LivingEntity target, float velocity) {
        this.performRangedAttack(target);
    }

    @Override
    protected int getClassId() {
        return ClassType.ARCHER.id;
    }
}
