package com.eeeab.eeeabsmobs.server.entity.effect.projectile;

import com.eeeab.eeeabsmobs.client.ControlledAnimation;
import com.eeeab.eeeabsmobs.server.entity.effect.EntityCameraShake;
import com.eeeab.eeeabsmobs.server.entity.effect.EntityExplode;
import com.eeeab.eeeabsmobs.server.entity.effect.IEntity;
...
public class EntityBloodBall extends Projectile implements IEntity {
    ...
    public void tick() {
        ...
        if (level().isClientSide || (owner == null || !owner.isRemoved()) && level().hasChunkAt(blockPosition())) {
            ...
            if (tickCount >= MAX_ACTIVE) preDestroy(null);

            if (level().isClientSide) {
                for (int i = 0; i < 2; ++i) {
                    double dx = getRandomX(0.5);
                    double dy = getRandomY();
    ...
    public float getBaseDamage() {
        return ModConfigHandler.COMMON.mobs.corpses.corpseWarlock.bloodBall.damage.get().floatValue();
    }
