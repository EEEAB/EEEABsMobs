package com.eeeab.eeeabsmobs.server.entity.mob.immortal;
...
import com.eeeab.eeeabsmobs.server.entity.ai.goal.owner.CopyOwnerTargetGoal;
import com.eeeab.eeeabsmobs.server.entity.ai.goal.owner.ReFindOwnerGoal;
import com.eeeab.eeeabsmobs.server.entity.ai.goal.owner.WhenOwnerDeadGoal;
...
public class EntityMagicGolem extends EntityAbsImmortal {
    ...
    protected ModConfigHandler.AttributeConfig getAttributeConfig() {
        return ModConfigHandler.COMMON.mobs.immortals.immortalGolem.combatConfig;
    }
    ...
    protected void registerCustomGoals() {
        ...
        this.targetSelector.addGoal(3, new HurtByTargetGoal(this, EntityAbsImmortal.class) {
            @Override
            public boolean canUse() {
                if (super.canUse()) {
                    LivingEntity entity = this.mob.getLastHurtByMob();
                    return entity != null && !this.mob.isAlliedTo(entity);
                }
                return false;
            }
        });
    ...
    public static AttributeSupplier.Builder setAttributes() {
        return createMobAttributes().add(Attributes.MAX_HEALTH, 12.0D).add(Attributes.MOVEMENT_SPEED, 0.4D).add(Attributes.FOLLOW_RANGE, 16.0D).add(Attributes.ATTACK_DAMAGE, 2.5D);
    }
